...
from scripts.airsim_env_magnet import rllib_env
from scripts.rllib_model_magnet import GNNPerAgentModelMAGNET

# Register custom model
ModelCatalog.register_custom_model("gnn_per_agent_model", GNNPerAgentModelMAGNET)
...
# Look for the most recent MAGNET checkpoint (fallback to legacy prefix)
import glob
magnet_ckpts = glob.glob(os.path.join("saved_policy", "magnet_iter_*"))
legacy_ckpts = glob.glob(os.path.join("saved_policy", "iter_*")) if not magnet_ckpts else []
pick_from = magnet_ckpts if magnet_ckpts else legacy_ckpts
if pick_from:
    def _iter_num(p):
        try:
            ...
            return -1
    latest_checkpoint = sorted(pick_from, key=_iter_num)[-1]
    checkpoint_dir = latest_checkpoint
    start_iteration = max(0, _iter_num(latest_checkpoint))
...
if SummaryWriter is not None:
    try:
        tb_writer = SummaryWriter(log_dir=os.path.join("tb_logs", "custom"))
        print("✓ TensorBoard custom writer initialized at tb_logs/custom")
    except Exception as _:
        tb_writer = None
        print("⚠️  Could not initialize custom TensorBoard writer; relying on RLlib logger only")
try:
        ...
        try:
            ...
            try:
                if tb_writer is not None:
                    ...
                    tb_writer.add_scalar('train/episode_reward_mean', er_mean if er_mean is not None else 0.0, i+1)
                    tb_writer.add_scalar('train/episode_len_mean', el_mean if el_mean is not None else 0.0, i+1)
                    if r_min is not None: tb_writer.add_scalar('train/episode_reward_min', r_min, i+1)
                    if r_max is not None: tb_writer.add_scalar('train/episode_reward_max', r_max, i+1)
                    tb_writer.add_scalar('train/env_steps_total', total_env_steps, i+1)
                    if (i + 1) % 5 == 0:
                        tb_writer.flush()
            except Exception:
                pass
            ...
            if (i + 1) % 5 == 0:
                save_dir = os.path.join("saved_policy", f"magnet_iter_{i+1:04d}")
    ...
    try:
        final_dir = os.path.join("saved_policy", f"magnet_final_{int(time.time())}")
